...
async function doScan(setSt, setPr) {
  ...
  const q = enc(
    "subject:(order confirmation OR ticket confirmation OR your tickets OR e-ticket) (ticketmaster OR seatgeek OR livenation OR eventbrite OR stubhub OR axs OR dice OR tickpick)",
  );
  const list = await (
    await fetch(
  ...
  setPr(40);
  setSt("Reading " + msgs.length + " emails…");
  const bodies = [];
...
function App() {
  ...
  const scanGmail = async () => {
    ...
    setScanPr(0);
    try {
      const r = await doScan(setScanSt, setScanPr);
      setScanPr(100);
      setScanSt(
        r.length
